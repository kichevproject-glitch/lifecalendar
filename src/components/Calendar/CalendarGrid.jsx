...
export default function CalendarGrid({ categories, sharedEvents = [] }) {
  ...
  function getTasksForDay(day) {
    return tasks
      .filter(t => t.due_date === format(day, 'yyyy-MM-dd') && t.status !== 'done')
  }
          ...
          return (
            ...
            >
              ...
              <div style={eventList}>
                ...
                {/* Shared events — 👥 replaces category icon */}
                {getSharedEventsForDay(day).slice(0, 1).map(ev => (
                  <div
                    key={'s-' + ev.id}
                    style={{
                      ...eventChip,
                ...
                {/* Tasks — dashed border, no icon, priority dot at end */}
                ...
                {(dayEvents.length + getSharedEventsForDay(day).length + getTasksForDay(day).length) > 5 && (
                  <div style={moreChip}>+{dayEvents.length + getSharedEventsForDay(day).length + getTasksForDay(day).length - 5} more</div>
                )}
              </div>
            </div>
          )
